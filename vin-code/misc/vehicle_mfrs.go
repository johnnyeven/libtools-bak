...
const UnknownMFRS = "Unknown"

// 制造商WMI
var vinMFRSMap = map[string]string{
	"AV ": "Volkswagen South Africa",
...
// 获取制造商
func GetVINManuf(vin string) string {
	wmi3 := vin[:3]
	wmi2 := vin[:2] + " "
	mf, ok := vinMFRSMap[wmi3]
	if !ok {
		mf, ok = vinMFRSMap[wmi2]
		if !ok {
			return UnknownMFRS
		}
	}
