...
func (key *Key) Add() *Expression {
	return Expr("ADD").ConcatBy(" ", key.Def())
}
...
func (key *Key) Def() *Expression {
	if key.Type == PRIMARY {
		return Expr(string(key.Type)+" KEY").ConcatBy(" ", key.Columns.Wrap())
	}
	return Expr(string(key.Type)+" "+key.String()).ConcatBy(" ", key.Columns.Wrap())
}
